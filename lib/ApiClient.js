...
import { JwtTokenExpiredError } from './XMindError.js';
import { getFormattedFiltersArray, chunk } from './Utils.js';
...
class ApiClient {
  ...
  #executeBulk(objPropertyName, array, chunkSize, endpoint, timeout) {
    return new Promise((resolve, reject) => {
      Promise.all(chunk(array, chunkSize)
        .map(chunkedArr => {
          let bodyParams = {};
  ...
  verifyAccount(code, email) {
    ...
    let path = '/accounts/verify/?' + JSON.stringify(queryParams);
  ...
  listUsersPaginated(amt = 300, cursor) {
    ...
    let path = '/users-bulk/?' + JSON.stringify(queryParams);
    return this.makeRequestWithAutoRefreshToken('GET', path);
  }
  ...
  listItemsPaginated(amt = 300, cursor) {
    ...
    if (cursor)
      queryParams['cursor'] = cursor;
    let path = '/items-bulk/?' + JSON.stringify(queryParams);
    return this.makeRequestWithAutoRefreshToken('GET', path);
  }
  ...
  listUserRatings(userId, page = 1, amt = 64) {
    ...
    let path = `/users/${userId}/ratings/?` + JSON.stringify(queryParams);
    return this.makeRequestWithAutoRefreshToken('GET', path);
  }
  ...
  listRatings(page = 1, cursor = null) {
    ...
    if (cursor)
      queryParams['cursor'] = cursor;
    let path = '/ratings-bulk/?' + JSON.stringify(queryParams);
    return this.makeRequestWithAutoRefreshToken('GET', path);
  }
