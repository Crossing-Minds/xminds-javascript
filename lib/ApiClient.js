...
class ApiClient {
   ...
   * 
   */
  #makeRequest(method, endpoint, bodyParams, timeout, options) {
    switch (method) {
      case 'GET':
      ...
      case 'POST':
        return this.#request.post(endpoint, bodyParams, timeout, options);
      case 'DELETE':
   ...
   */
  async loginRefreshToken(refreshToken = '', dbId = null) {
    let bodyParams = { 'refresh_token': refreshToken !== '' ? refreshToken : this._getRefreshToken() };
    if (dbId)
    ...
    this._setRefreshToken(authData.refresh_token);
    return authData;
  }
   ...
   * 
   */
  createAnonymousSessionInteraction(sessionId, itemId, interactionType, timestamp = null) {
    let bodyParams = {};
    bodyParams['interaction_type'] = interactionType;
    if (timestamp)
      bodyParams['timestamp'] = timestamp;
    let path = `/sessions/${sessionId}/items/${itemId}/interactions/`;
    return this.makeRequestWithAutoRefreshToken('POST', path, bodyParams, undefined, { keepalive: true });
  }
